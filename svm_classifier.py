...
print("开始训练SVM分类器...")
# 初始化并训练SVM分类器
svm = SVC(kernel='rbf', C=1.0, random_state=42, verbose=2, max_iter=1000)  # 添加verbose参数
svm.fit(X_train, y_train)
print(f"SVM分类器训练完成, 耗时: {time.time() - st} 秒")
...
# 保存结果到文件
result_path = os.path.join(RESULT_DIR, 'SVM_Results.txt')
with open(result_path, 'w') as f:
    f.write(f'SVM分类器准确率: {accuracy * 100}%\n')

print(f'SVM分类器准确率: {accuracy * 100}%')
