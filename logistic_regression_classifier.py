...
print(f"逻辑回归分类器训练完成, 耗时: {time.time() - st} 秒")
...
# 保存结果到文件
result_path = os.path.join(RESULT_DIR, 'LogisticRegression_Results.txt')
with open(result_path, 'w') as f:
    f.write(f'逻辑回归分类器准确率: {accuracy * 100}%\n')

print(f'逻辑回归分类器准确率: {accuracy * 100}%')
